...
uiSection:AddButton({
    ...
    Callback = function()
        local info = EZ:CheckForUpdate("Aiven/EZ")
        if info then
            EZ:Notify({
                Title = "Update",
...
previewSection:AddParagraph({
    ...
    Content = "Tap any below to apply instantly. EZ ships 9 curated dev-favorite themes.",
})
...
local function fakeWork(label, dur)
    EZ:Notify({ Title = label, Content = "running...", Duration = dur or 2, Type = "info" })
end
